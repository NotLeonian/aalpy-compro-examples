...
各 DFA は、`--kind learn` による能動オートマトン学習または `--kind regex` による正規表現からの変換によって生成させます。  
このとき、使用者は各 DFA について他の DFA と被らない一意なキーを `--key` オプションに続けて指定する必要があります。

`--kind learn` では、能動オートマトン学習を行えます。  
使用者はアルファベットおよび受理判定の関数を、Python のプログラム（ソースコード）として与える必要があります。  
また、使用者は使用するオラクル (`wp`, `random_wp`, `state_prefix` のどれか 1 つ) を `--oracle` で与えるか、その代わりにオートマトン学習に使用する語の一覧を `eq_words` (語のリスト) または `iter_eq_words` (語が `yield` 文の返り値として列挙された関数) で与える必要があります。

`--kind regex` では、正規表現から等価な最小 DFA への変換が行えます。  
使用者はアルファベットおよび正規表現を、Python のプログラム（ソースコード）として与える必要があります。
