...
また、使用者は学習に使用するオラクル（`wp`, `random_wp`, `state_prefix` のどれか 1 つ）を `--oracle` に続けて与えるか、学習中に受理判定などに使用する語の一覧を `eq_words`（語のリスト）または `iter_eq_words`（語が `yield` 文の返り値として列挙された関数）として `--path` に与えた Python のソースコード内に記述する必要があります。  
`--oracle` オプションと `eq_words` または `iter_eq_words` の記述は両立できますが、`eq_words` と `iter_eq_words` の両方を記述することはできません。
