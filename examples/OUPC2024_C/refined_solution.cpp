...
int main(void) {
    ...
    std::fill(dp_table.begin(), dp_table.end(), NEG_INF);
    ...
    for (int i = 0; i < n; i += 1) {
        ...
        std::fill(next_table.begin(), next_table.end(), NEG_INF);
        next_table[dfa.index_of_initial_state()] =
            dp_table[dfa.index_of_initial_state()];
        for (int j = 0; j < dfa.state_size(); j += 1) {
            int cand = dp_table[j] + 1;
        ...
        for (int j = 0; j < dfa.state_size(); j += 1) {
            if (dfa.is_accepting(j)) {
                int cand = dp_table[j];
                int &target = dp_table[dfa.index_of_initial_state()];
    ...
    std::cout << dp_table[dfa.index_of_initial_state()] << "\n";
