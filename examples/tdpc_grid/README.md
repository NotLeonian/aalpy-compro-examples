...
この問題に対して得られる DFA は状態数と文字集合の要素数の差は小さいが、状態数が文字集合の要素数以上であるため疎行列を返す `sparse_transition_count_matrix` 関数を使用している。
...
疎行列の場合、[Nyaan's Library](https://nyaannyaan.github.io/library/) などを参考に Black Box Linear Algebra で値を求めると高速である。

これらを踏まえ、learned_dfa.cpp に main 関数などを追加したのが [refined_solution.cpp](./refined_solution.cpp) である。  
Black Box Linear Algebra 等の実装は [Nyaan's Library の実装](https://nyaannyaan.github.io/library/matrix/black-box-linear-algebra.hpp)を改変した。
...
## 提出結果
[AC 提出 (C++23 (GCC 15.2.0), 3 ms)](https://atcoder.jp/contests/tdpc/submissions/74168214)
